...
from exits import (_record_session_exit, crash_recovery_verdict, ending_was_observed,
                   handoff_state, HANDOFF_MISSING, HANDOFF_WAITING)
            ...
            while session_num <= MAX_SESSIONS:
                if adopting:
                    ...
                    launch_preamble = build_preamble(
                        agent, instance,
                        # Both branches read the one probe above. Probing twice
                        # would let the preamble's two handoff clauses disagree
                        # with each other and with the record just written.
